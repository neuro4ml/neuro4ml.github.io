...
Previously we explained how when a spike arrives at a transmitting synapse, a [neurotransmitter](neurotransmitters-paragraph) is released. This neurotransmitter causes a change in the conductance of the receiving cell due to the opening of an associated ion channel. In turn, this leads to an input current following the same sort of equations we saw for the [Hodgkin-Huxley](hodkin-huxley-label) formalism last week. This then leads to a change in membrane potential for either the
...
```{figure} synapse-modelsPicture10.png
:width: 500px
:align: center
```

So what does this do in terms of function? As usual, we don’t have a complete answer to that. One thing for sure is that it gives synapses a longer memory than they would otherwise have, which is likely to be useful in many cases.
...
W(\delta t) = 
\begin{cases}
    A_+ e^(-\frac{\delta t}{\tau_+})    for \delta t > 0 \\
    A_- e^(\frac{delta t}{\tau_-})      for \delta t < 0
\end{cases}
```
