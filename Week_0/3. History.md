...
The next development I have to talk about is Minsky’s criticisms of the perceptron. We won’t say much about this since it doesn’t bear directly on neuroscience, but it feels wrong not to mention it at all.
...
Minsky thought that multilayer neural networks would be hard to train, but Rumelhart, Hinton and Williams found a solution in backpropagation.
...
But in a fascinating development from 2016 onwards, Timothy Lillicrap and colleagues found that if you just use a random feedback network that isn’t symmetrical, it still works!
...
In the late 50s and early 60s, neuroscientists Hubel and Wiesel discovered [two types of cells](#2t) dominated in the early part of the visual system.
...
In [1989 Yann LeCun](https://doi.org/10.1162/neco.1989.1.4.541) and others took essentially the same architecture but trained it using backpropagation, later refining this model and calling it a convolutional neural network.
...
In 2014 Dan Yamins and colleagues found that training a {abbr}`CNN(Convolutional Neural Network)` to perform well at a task actually gave internal representations that were a close match to neural data recorded in the visual system.

Alex Kell and colleagues found the same thing in the auditory cortex.

This led Martin Schrimpf and others to propose a “brain score” and leaderboard to track which models gave the closest match to neural data.

But the story isn’t over, and there are lots of people out there who are not convinced by this:

* Jacob et al. found that these networks don’t predict the patterns of success and failure made by humans on out-of-distribution stimuli.
...
Incidentally, the Thorndike paper (1898) is [pretty funny](#funny) because it seems his main aim was to prove that animals are actually stupid, which I guess was an active debate in that day.
...
* Like [Alan Turing’s 1948 paper](#alan) where he describes a computational learner that is explicitly described as a model of cortex.
